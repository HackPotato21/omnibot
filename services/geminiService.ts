...
// --- Gen Z Persona System Instruction ---
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        temperature: 0.8, // A bit more creative for a sarcastic persona
      },
    });

    const text = response.text;
    return { text, sources: [] }; // No sources for this implementation
  } catch (error) {
    console.error("Gemini API call failed:", error);
    // Return a persona-aligned error message
